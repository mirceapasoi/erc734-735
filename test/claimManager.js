...
contract("ClaimManager", async (accounts) => {
            ...
            [
                {
                    type: Topic.LABEL,
                    data: "Mircea Pasoi",
                    uri: "http://mirceapasoi.com",
                    self: true,
                },
                {
                    type: Topic.LABEL,
                    data: "Mircea Bogdan Pasoi",
                    uri: "",
                    self: false,
                },
            ]
    ...
    describe("addClaim", () => {
        ...
        it("can add self-claim as manager", async () => {
            ...
            await assertOkTx(
                identity.addClaim(Topic.PROFILE, Scheme.ECDSA, identity.address, signature, label, uri, {
                    from: addr.manager[0],
                })
            );
        ...
        it("checks signature when adding", async () => {
            ...
            await expectRevert(
                identity.addClaim(Topic.PROFILE, Scheme.ECDSA, identity.address, invalidSignature, label, uri, {
                    from: addr.manager[0],
                }),
                "invalid signature"
            );
            ...
            await expectRevert(identity.getClaim(claimId), "issuer must exist");
        });
        ...
        it("other identity can add a claim", async () => {
            ...
            let executeData = identity.contract.methods
                .addClaim(Topic.PROFILE, Scheme.ECDSA, otherIdentity.address, signature, label, uri)
                .encodeABI();
            let r = await assertOkTx(
                otherIdentity.execute(identity.address, 0, executeData, {
                    from: addr.other,
                })
            );
            let claimRequestId = findClaimRequestId(r);
    ...
    describe("changeClaim", () => {
        it("can update a self-claim", async () => {
            ...
            await assertOkTx(
                identity.changeClaim(claimId, Topic.LABEL, Scheme.ECDSA, identity.address, signature, label, newUri, {
                    from: addr.manager[1],
                })
            );
        ...
        it("needs approval to update a self-claim", async () => {
            ...
            // Use same signature to update URI
            let r = await assertOkTx(
                identity.addClaim(Topic.LABEL, Scheme.ECDSA, identity.address, signature, label, newUri, {
                    from: addr.claim[0],
                })
            );
            let claimRequestId = findClaimRequestId(r);
        ...
        it("other identity can update a claim", async () => {
            ...
            let executeData = identity.contract.methods
                .changeClaim(claimId, Topic.LABEL, Scheme.ECDSA, otherIdentity.address, signature, data, newUri)
                .encodeABI();
            await assertOkTx(
                otherIdentity.execute(identity.address, 0, executeData, {
                    from: addr.other,
    ...
    describe("refreshClaim", () => {
        ...
        it("removes claims that are invalid", async () => {
            ...
            await assertOkTx(
                identity.removeKey(keys.claim[0], Purpose.CLAIM, {
                    from: addr.manager[0],
                })
            );
            ...
            await assertOkTx(
                identity.execute(identity.address, 0, executeData, {
                    from: addr.manager[1],
                })
            );
